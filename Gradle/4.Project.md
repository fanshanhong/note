...
project(':app') 是在根工程中查找名字为app的子工程。  返回值就是 app 子工程对象，然后修改这个子工程的 projectDir 属性值。这样就可以了。

# Build文件


每个 Project 都有一个 build.gradle 文件。
每一个 build.gradle 脚本文件被 Gradle 加载解析后，都会对应生成一个 Project 对象。
...
访问的时候， 任务名就是key。
其实，`[]`在Grovvy中是一个操作符。 `a[b]`对应的 是 `a.getAt(b)`这个方法。对应的，我们`tasks['hello']`其实就是调用`tasks.get('hello')`这个方法，查看Gradle源码，可以看到，这个方法是通过`tasks.findByName(String  name) `来实现的。
