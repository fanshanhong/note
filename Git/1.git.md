...
git pull 命令的作用是将远端仓库的远端分支下载到本地， 并将下载的远端分支合并到本地分支中。 它的 用法如下： git pull <远端仓库> <远端分支>或 git pull
...
Git 的历史记录是由一系列彼此关联的提交(commit)记录构成的。与传统的版本管理工具不同，git 不是通过版本号来表示提交记录的。相反 Git 为每一个提交 记录分配一个 40 位的 16 进制数字来学标识它，不难理解，这串数字一定是唯一的。
