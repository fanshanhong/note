 ...
 * ways to manage stateful information.
 Context 的 AttributeKey 允许你存储和访问与 handler 和  context 关联的 一些状态信息。
 ...
 * same pipeline multiple times or added to different pipelines multiple times:
 ...
 但是我看代码，AttributeKey怎么是和Channel 关联的？？ 如果是和channel 关联的， 那一个pipeline里面所有的Context 都是同一个AttributeKey了。
 需要再研究一下。
 
 好像是：在老版本里面，attr是存在Context里的。
 在4.1.45的版本里面，已经废弃了。让使用Channel.attr()方法了。代码贴在这里了
   ...
   AbstractChannelHandlerContext 中attr()方法的 具体实现：是返回了channel的attr
    ...
    大概看了一下， channel的attr 是个array， 那我们可以理解为：  这个array 的每一个元素， 是和 pipline里面的每一个 Context 一一对应的。
    
    pipeline ：     					context1        			 --->      context2                 --->      context3
    channel的attr是个array： context1的AttributeKey --->    context2的AttributeKey		  --->      context3的AttributeKey
