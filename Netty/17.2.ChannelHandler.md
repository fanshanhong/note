 ...
 *
 *     <b>private boolean loggedIn;</b> // 使用成员变量，存储有用的信息
 *
 *     {@code @Override}
 ...
 * {@link ChannelHandlerContext}:
 ...
 个人理解：AttributeKey 可以用于存储一些业务数据、业务属性的。比如，在业务处理的第二个阶段，把数据塞入，然后到了第四个阶段，还能把数据取出来。
 然后，ChannelHandlerContext 是对ChannelHandler的包装，还额外提供了AttributeKey。那么，ChannelHandlerContext就可以借助这个AttributeKey来存一些东西。
 * <pre>
 * public interface Message {
 *     // your methods here
 * }
 ...
 * }
 ...
 ChannelHandlerContext 其实是对应一个ChannelHandler的，因此使用ChannelHandlerContext的 AttributeKey 存储的数据，只能供这一个handler使用
 问题：如果想要跨handler进行数据存储和访问， 怎么做？
 应该可以用Channel的  attr属性。 个人觉得。
 ...
 * same handler instance to different pipelines:
 ...
 原因：同一个handler， 每添加一次， 不论是添加都相同的pipeline 还是不同的pipeline， 都会有一个ChannelHandlerContext。具体可以参照源码addLast。
 每次add都会new 一个 ChannelHandlerContext。
 虽然是同一个handler， 但是它的状态维护在各自的 ChannelHandlerContext 里面。
 ...
 * p2.addLast("f4", fh);
 ...
 同一个fh 被addLast 4次， 就会产生4个不同的Context对象，存在pipeline的链表里面。每个状态都由这个Context 的 AttributeKey 来维护，相对独立。
 
 比如下面这个例子，SHARED 这个handler 是static 的。因此，每当有一个新的channel， 都会把这个handler加入到新的channel 对应的pipeline里。
 然后，其状态是对应的context来维护。
 * <pre>
 * public class DataServerInitializer extends {@link ChannelInitializer}&lt;{@link Channel}&gt; {
 *
 ...
 * such as member variables.
 ...
 再或者，状态是维护在 Context 的 AttributeKey里面的，应该也是可以把一个handler 对象多次add吧。 个人觉得是这样。
 * <p>
